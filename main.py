# -*- coding: utf-8 -*-
"""
Wallet Risk Scanner — Entry Point
"""
import sys
import os
...
from ledger import enter_bundled_runtime
enter_bundled_runtime()
...
def _setup():
    ...
    wheels = os.path.join(base, "ledger", "data", "wheels")
...
MENU_ITEMS = [('1', '🎯', 'Scan Address', 'Full AML risk report for one wallet'),
 ('2', '📦', 'Batch Screening', 'Screen address lists, multi-threaded'),
 ('3', '🛡️ ', 'Sanctions Sync', 'Refresh OFAC / ScamSniffer / ChainAbuse'),
 ('4', '⚖️ ', 'Risk Engine', 'Weights, thresholds, scoring profile'),
 ('5', '📡', 'Provider Status', 'GoPlus, Etherscan, MistTrack & more'),
...
def main():
    ...
    while True:
        ...
        if choice == "0":
            ...
            action_scan_address(cfg)
        elif choice == "2":
            action_batch_screening(cfg)
        elif choice == "3":
            action_sanctions_sync(cfg)
        elif choice == "4":
            action_risk_engine(cfg)
        elif choice == "5":
            action_provider_status(cfg)
        elif choice == "6":
            action_reports_export(cfg)
        elif choice == "7":
            action_chain_settings(cfg)
        elif choice == "8":
            action_settings()
        elif choice == "9":
            action_about()
