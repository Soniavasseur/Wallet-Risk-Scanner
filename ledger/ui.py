# -*- coding: utf-8 -*-
"""
Rich console UI components for Wallet Risk Scanner.
"""
...
def print_banner():
    ...
    console.print(
        Panel(
            ...
            "[dim]Multi-Service AML & Risk Screening Engine[/dim]",
...
def progress_bar(description, total=100, transient=False):
    return Progress(
        ...
        BarColumn(bar_width=40, complete_style="bright_red", finished_style="bright_green"),
...
def show_simple_list(title, items):
    ...
    if title:
        console.print(f"\n[bold bright_red]{title}[/bold bright_red]")
    for item in items:
